...
window.imgIchigoAttack.src = 'skillIMG/dan_ban_thuong_ichigo.png';
window.imgIchigoQSprite = new Image();
window.imgIchigoQSprite.src = 'skillIMG/anh_chieu_q.png';       
window.imgIchigoE = new Image();
window.imgIchigoE.src = 'skillIMG/dam_cuong_hoa_e_ichigo.png';
window.imgIchigoEShadow = new Image();
window.imgIchigoEShadow.src = 'skillIMG/du_anh_e_ichigo.jpg';
window.imgIchigoSpace = new Image();
window.imgIchigoSpace.src = 'skillIMG/chem_space.jpg';
...
class Bullet {
    ...
    update() {
        ...
        let isPiercing = this.type.includes('clone') || this.type === 'pika_e' || this.type === 'ichigo_q' || this.type === 'ichigo_knife' || this.type === 'thachsanh_giant_arrow';
        if (this.x < 0 || this.x > WORLD_W || this.y < 0 || this.y > WORLD_H || (!isPiercing && hitWall) || this.distTraveled > this.maxRange) {
            this.active = false;
    ...
    draw() {
        ...
        if (this.type === 'player') {
            ...
            ctx.rotate(this.angle);
            ctx.fillStyle = '#2c3e50';
            ctx.fillRect(-this.radius * 6.2, -3.2, this.radius * 4.4, 7.4);
            ctx.fillStyle = '#00e5ff';
            ctx.shadowBlur = 12;
            ctx.shadowColor = '#00e676';
            ctx.beginPath();
            ctx.moveTo(this.radius * 1.6, -8);
            ctx.lineTo(this.radius * 3.0, 0);
            ctx.lineTo(this.radius * 1.6, 8);
            ctx.closePath();
            ctx.fill();
            ctx.beginPath();
            ctx.moveTo(this.radius * 0.4, -13);
            ctx.quadraticCurveTo(this.radius * 1.6, -16, this.radius * 2.0, -3);
            ctx.lineTo(this.radius * 1.5, -3);
            ctx.quadraticCurveTo(this.radius * 1.1, -9, this.radius * 0.4, -6);
            ctx.closePath();
            ctx.fill();
            ctx.beginPath();
            ctx.moveTo(this.radius * 0.4, 13);
